...
public:
	...
	{
	public:
		inline Concatenator(const PString &s1, const PString &s2);
		inline Concatenator &operator+=(const PString &s);
		inline Concatenator &operator+(const Concatenator &other);
		inline Concatenator &operator+(const PString &s);
		inline operator PString() const;

	private:
		unsigned char m_index;
		const PString *m_data[10];
		PString::size_type m_total;
	};
...
PString::Concatenator::Concatenator(const PString &s1, const PString &s2) :
	m_index(1),
...
PString::Concatenator &PString::Concatenator::operator+=(const PString &s)
{
	m_data[++m_index] = &s;
	m_total += s.size();
	return *this;
...
PString::Concatenator &PString::Concatenator::operator+(const Concatenator &other)
{
...
PString::Concatenator::operator PString() const
{
	...
	return PString::fromByteArray(res);
}


inline PString::Concatenator operator+(const PString &s1, const PString &s2)
{
	return PString::Concatenator(s1, s2);
}
